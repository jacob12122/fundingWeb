package shanghai.shu.edu.mvc.config;
...
//			.passwordEncoder(passwordEncoder)
	...
	protected void configure(HttpSecurity security) throws Exception {
		...
		security
			...
			.access("hasAuthority('user:get')")
			.antMatchers("/admin/remove*/*")
			.access("hasAuthority('user:delete')")
			.antMatchers("/role/to/page.html")
			.access("hasAuthority('role:get')")
			.antMatchers("/role/remove*/*")
			.access("hasAuthority('role:delete')")
			.antMatchers("/role/to/add*/*")
			.access("hasAuthority('role:add')")
			.anyRequest()					// 其他任意请求
			.authenticated()				// 认证后访问
			.and()
			.exceptionHandling()
